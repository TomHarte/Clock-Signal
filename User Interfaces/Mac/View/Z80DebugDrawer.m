...
- (id)init
{
	self = [super initWithContentSize:NSSizeFromCGSize(CGSizeMake(212, 468)) preferredEdge:NSMinXEdge];

	if(self)
	{
		self.maxContentSize = self.minContentSize = self.contentSize;

//		self.memoryCommentLines = [NSMutableArray array];
	}
...
- (void)awakeFromNib
{
	self.maxContentSize = self.minContentSize = self.contentSize = [self.contentView bounds].size;
...
{
	[delegate debugDrawerRunForOneInstruction:self];
}

- (IBAction)runForHalfACycle:(id)sender
{
	[delegate debugDrawerRunForHalfACycle:self];
}
...
- (IBAction)runUntilAddress:(id)sender
{
	...
	[delegate debugDrawer:self runUntilAddress:address];
}

- (IBAction)run:(id)sender
{
	[delegate debugDrawerRun:self];
}

- (IBAction)pause:(id)sender
{
	[delegate debugDrawerPause:self];
}
...
- (void)set8BitValue:(LLZ80MonitorValue)value toField:(NSTextField *)field
{
	[self
		setString:
			[NSString stringWithFormat:@"%02x", llz80_monitor_getInternalValue(delegate.z80ForDebugDrawer, value)]
			toField:field];
}
...
- (void)set16BitValue:(LLZ80MonitorValue)value toField:(NSTextField *)field
{
	[self
		setString:
			[NSString stringWithFormat:@"%04x", llz80_monitor_getInternalValue(delegate.z80ForDebugDrawer, value)]
			toField:field];
}
	...
	[self
		setString:
			[NSString stringWithFormat:@"%04x",
				llz80_monitor_getInternalValue(delegate.z80ForDebugDrawer, lowValue) |
				(llz80_monitor_getInternalValue(delegate.z80ForDebugDrawer, highValue) << 8)
			]
			toField:field];
}
...
- (void)updateBus
{
	unsigned int z80Time = llz80_monitor_getInternalValue(delegate.z80ForDebugDrawer, LLZ80MonitorValueHalfCyclesToDate);
	
	if(z80Time == lastBusTime) return;
	lastBusTime = z80Time;

	uint64_t busState = llz80_monitor_getBusLineState(delegate.z80ForDebugDrawer);
...
- (void)refresh
{
	...
	uint8_t flagRegister = llz80_monitor_getInternalValue(delegate.z80ForDebugDrawer, LLZ80MonitorValueFRegister);
	[self setString:
	...
	unsigned int newInternalTime = llz80_monitor_getInternalValue(delegate.z80ForDebugDrawer, LLZ80MonitorValueHalfCyclesToDate);
...
+ (id)debugDrawer
{
	id returnDrawer = [[[[self class] alloc] init] autorelease];
	[NSBundle loadNibNamed:@"Z80DebugDrawer" owner:returnDrawer];
	return returnDrawer;
}
